<template>
    <div
        ref="poi"
        class="relative point-of-interest-content"
        :oid="point.target?.oid"
        :layerId="point.target?.layerId"
        :layerIndex="point.target?.layerIndex"
    >
        <!-- Point Icon -->
        <div class="flex">
            <span
...
onMounted(() => {
    ...
    const poiObserver = new IntersectionObserver(
        ([point]) => {
            ...
            // Grab the HTML element from the IntersectionObserverEntry.
            const p = point.target;

            // Get the attributes from the element.
            const oid = p.getAttribute('oid');
            const layerId = p.getAttribute('layerId');
            const layerIndex = p.getAttribute('layerIndex');

            if (point.isIntersecting) {
                emit('poi-changed', layerId, oid, layerIndex);
            }
        },
