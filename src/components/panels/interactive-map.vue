<template>
    <div ref="el" class="h-full align-middle w-full interactive-container sticky">
        ...
        <div
            ...
            class="w-full bg-gray-200 h-story rv-map sticky top-16 interactive-content"
        ></div>
    </div>
</template>
        ...
        data.json().then((rampConfig: any) => {
            ...
            mapInstance.geo.map.viewPromise.then(() => {
                props.config.points.forEach(async (point: PointOfInterest) => {
                    const layer = mapInstance.geo.layer.getLayer(point.target.layerId);
                    layer.loadDefProm.realPromise.then(async () => {
                        layer.getIcon(point.target.oid).then((icon: any) => {
...
const handlePoint = (id: string, oid: number) => {
    if (!id || !oid) return;
    ...
    rInstance.value.geo.map.viewPromise.then(() => {
        ...
        layer.loadDefProm.realPromise.then(async () => {
            ...
            const g = await layer.getGraphic(oid, { getGeom: true, getStyle: true });
