import{p0 as A,q3 as E,p4 as R,p5 as j,pf as T,p6 as _,q5 as C,ph as V,p2 as X,pg as q,pi as F,p7 as U,p8 as n,p9 as D,pa as k,pP as G,pp as c,pd as g,pk as d,pb as h}from"./story-u82SUoyt.js";import{u as B}from"./marked.esm-DgqJBp_S-CCvGNfAT.js";import"./main-CmCzAIzx.js";const Z=`<?xml version="1.0" encoding="UTF-8"?>
...
`,J={key:0},K={class:"flex justify-center"},Q={key:0,class:"flex flex-col justify-center text-center"},W=["innerHTML"],Y=["innerHTML"],ee={key:3,class:"flex flex-col justify-center text-center"},te={class:"text-xl mt-20"},ae={key:1,class:"p-5"},ne=A({__name:"screen",props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(r){const s=E(),{t:o}=R(),u=j("iApi"),I=T(),l=r,f=_(()=>s.status),b=_(()=>s.response),v=T(!1),x=C({}),y=C([]),$=C([]);V(()=>{S(),y.push(u.event.on(X.LAYER_REMOVE,e=>{l.payload.layer?.uid===e.uid&&l.panel.close()})),$.push(q(()=>l.payload.layer.uid,(e,t)=>{e!==t&&S()}))}),F(()=>{y.forEach(e=>u.event.off(e)),$.forEach(e=>e())});const S=()=>{v.value=l.payload.layer!==void 0&&!l.payload.layer.isRemoved,l.payload.type==="xml"?M(l.payload.url,[]).then(e=>{if(s.status="success",e!==null){const t=document.createElement("div");t.appendChild(p(`${e.firstElementChild.outerHTML}`)),(l.payload.catalogueUrl||l.payload.url)&&t.appendChild(p(`<h5 class="text-xl font-bold mb-3">${o("metadata.xslt.metadata")}</h5>`)),l.payload.catalogueUrl&&t.appendChild(p(`<p><a style="color: blue;" href="${l.payload.catalogueUrl}" target="_blank">${o("metadata.xslt.cataloguePage")}</a></p>`)),t.appendChild(p(`<p><a style="color: blue;" href="${l.payload.url}" target="_blank">${o("metadata.xslt.metadataPage")}</a> (xml)</p>`)),s.response=t.outerHTML}}):l.payload.type==="html"?w(l.payload.url).then(e=>{s.status=e.status,s.response=e.response}):l.payload.type==="md"&&w(l.payload.url).then(e=>{s.status=e.status,s.response=B(e.response)})},M=(e,t)=>{let a=u.language==="en"?Z:z;return a=a.replace(/\{\{([\w.]+)\}\}/g,(i,m)=>o(m)),x[e]?Promise.resolve(P(x[e],a,t)):w(e).then(i=>(x[e]=i.response,P(x[e],a,t)))},P=(e,t,a)=>{let i=null;if(window.XSLTProcessor){const m=new window.XSLTProcessor,L=new DOMParser,N=L.parseFromString(e,"text/xml"),H=L.parseFromString(t,"text/xml");m.importStylesheet(H),a&&a.forEach(O=>m.setParameter("",O.key,O.value||"")),i=m.transformToFragment(N,document)}return i},w=e=>new Promise(t=>{const a=new XMLHttpRequest;a.open("GET",e,!0),a.responseType="text",a.onload=()=>{a.status===200?t({status:"success",response:a.response}):t({status:"error",response:"Could not load results from remote service."})},a.onerror=()=>{t({status:"error",response:"Could not load results from remote service."})},a.send()});function p(e){const t=document.createElement("div");return t.innerHTML=e,t}return(e,t)=>{const a=U("panel-screen");return n(),D(a,{panel:r.panel,ref_key:"el",ref:I},{header:k(()=>[G(c(g(o)("metadata.title"))+": "+c(r.payload.layerName),1)]),content:k(()=>[v.value?(n(),d("div",J,[h("div",K,[f.value=="loading"?(n(),d("div",Q,c(g(o)("metadata.loading")),1)):r.payload.type==="xml"&&f.value=="success"?(n(),d("div",{key:1,innerHTML:b.value,class:"flex flex-col justify-center xml-content"},null,8,W)):(r.payload.type==="html"||r.payload.type==="md")&&f.value=="success"?(n(),d("div",{key:2,innerHTML:b.value,class:"flex flex-col justify-center max-w-full metadata-view"},null,8,Y)):(n(),d("div",ee,[t[0]||(t[0]=h("img",{src:"https://i.imgur.com/fA5EqV6.png"},null,-1)),h("span",te,c(g(o)("metadata.error")),1)]))])])):(n(),d("div",ae,[h("span",null,c(g(o)("metadata.label.no.layer")),1)]))]),_:1},8,["panel"])}}});export{ne as default};
